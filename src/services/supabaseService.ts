...
export const userService = {
  ...
  async create(user: any): Promise<DbUser> {
    const { data, error } = await supabase
      .from('users')
      .insert([user])
      .select()
...
export const listingService = {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        users:seller_id(full_name, avatar_url, verified, verification_type, phone_number, business_name)
      `)
      .order('created_at', { ascending: false });
  ...
  async create(listing: any, imageUrls: string[]): Promise<any> {
    const { data, error } = await supabase
      .from('listings')
      .insert([listing])
      .select()
...
export const messageService = {
  async getConversations(userId: string): Promise<any[]> {
    const { data, error } = await supabase
      ...
      .select(`
        *,
        listings:listing_id(id, title, price, listing_images(image_url)),
...
export const passwordRequestService = {
  ...
  async create(req: any) { 
    await supabase.from('password_change_requests').insert([req]); 
  },
  async updateStatus(id: string, status: string) { 
...
export const promotionService = {
  ...
  async create(req: any) { 
    await supabase.from('promotion_payments').insert([req]); 
  },
  async updateStatus(id: string, status: string) { 
...
export const disputeService = {
  ...
  async create(disp: any) { 
    await supabase.from('disputes').insert([disp]); 
  },
  async updateStatus(id: string, status: string) { 
