      ...
      <main className="max-w-7xl mx-auto w-full px-4 py-6 flex-1 space-y-6">
        ...
        <div className="bg-slate-900 border-2 border-rose-500/30 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="space-y-2 max-w-xl">
            <div className="inline-flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-400 text-[10px] font-mono font-black px-3.5 py-1 rounded-full uppercase tracking-widest shadow">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          onClick={() => updateListing(ad.id, { featured: !ad.featured })}
                  ...
                  <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-slate-800 hover:bg-slate-750 text-slate-200 font-bold rounded-xl text-xs border border-slate-700 transition-colors"
                      >
                ...
                {verificationRequests.map((req) => (
                  <div key={req.id} className="p-5 bg-slate-900 border border-slate-800 rounded-3xl space-y-4 shadow-xl">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                      ...
                      <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-full ${req.status === 'pending' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : req.status === 'approved' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-600/20 text-rose-400'}`}>
                        {req.status}
                      </span>
                    </div>
              ...
              <div className="bg-slate-900 border border-rose-500/30 rounded-3xl p-6 space-y-4 shadow-xl">
                ...
                <form onSubmit={handleUpdateAdminPass} className="space-y-3 text-xs">
                  <div className="space-y-1">
                    <label className="text-slate-400 font-bold uppercase text-[10px]">Admin Login Email</label>
                    <input
                      type="email"
                      required
                      value={newAdminEmail}
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-bold uppercase text-[10px]">Admin Access Password</label>
                    <input
                      type="text"
                      required
                      value={newAdminPass}
                  ...
                  <div className="space-y-1">
                    <label className="text-slate-400 font-bold uppercase text-[10px]">6-Digit Master Security PIN</label>
                    <input
                      type="text"
                      required
                      maxLength={6}
